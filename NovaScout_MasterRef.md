...
settings
  key (PK: operator_email), value, updated_at

claims_library
  code (PK), slot (problem|outcome|proof|ask|link), body, countries[],
  measured (bool), active (bool), confirmed (bool), note, updated_at
```
...
**Prompt constraints:** under 80 words, minimum two specific facts, one geography-matched live customer (drafting skill v2; this replaced "mention the live NoblePath demo"), no adjectives like "revolutionary" or "cutting-edge", no merge-tag phrasing tells.
...
| Confirm the claims library (`claims_library`, migration `010`). Every line was seeded `confirmed=false` from skill §4 ("DRAFT, human confirms before first use"), and drafts built from them carry `unconfirmed-claim` | Both | Removing the `unconfirmed-claim` tag from first-touch drafts |
