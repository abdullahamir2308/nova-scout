...
| Get the manual warm-up sends into the mailbox's Sent folder (0 messages there on 2026-09-14), then Publish Mailbox Watch in the UI | Abdullah | Every Workflow 6 send — the send path refuses until the Sent mirror has read the folder |
| Set `NOVASCOUT_OPERATOR_EMAIL` (not the outreach mailbox) and rebuild | Abdullah | Workflow 6 reply notifications |
