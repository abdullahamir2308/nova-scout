...
1. **Override the default `presence_penalty`.** Qwen3.5 ships with `presence_penalty: 1.5`. **Tested correction:** the failure mode is NOT malformed JSON as originally guessed here — Ollama's grammar-constrained `format: json` mode makes syntactically invalid output impossible, which masks the problem. The real failure is silent, field-level data loss on copy-from-source fields: at 1.5, `founder_linkedin` and `therapeutic_areas` entries came back null/truncated even when clearly present in the source text, because the penalty discourages "repeating" tokens the model is meant to be copying, not generating. Output is valid-looking JSON with quietly missing data — more dangerous than a visible parse error, since it can pass silently into scoring. At `presence_penalty: 0`, all fields extracted correctly. For extraction and scoring nodes set `presence_penalty: 0`, `temperature: 0.1`. For drafting, `temperature: 0.45`, `presence_penalty: 0.3`.

**Drafting settings TESTED in Sprint 4 — the temperature came down from the 0.7 originally guessed here.** Measured on the real drafting prompt against three real leads at 0.7, 0.45 and 0.35. `format` schema enforcement holds at every temperature tried, so the choice is purely about content. 0.7 produced two failures 0.45 did not: an outright invention ("using that material and our own staff data" — no such thing exists) and a fabricated join ("you run a recruiting trial in Argentina", where the country is not even a supplied fact). It also dropped required sentences and, on one lead, inverted the product — "You run an assistant trained on your SOPs", i.e. telling the prospect they already own the thing being sold. 0.35 was structurally stable but noticeably repetitive across leads. 0.45 keeps the five-sentence shape and produced no inventions across the same set. `presence_penalty: 0.3` was left alone — unlike extraction, drafting is not copying long strings out of the prompt, which is the failure the extraction setting exists to prevent.

**The bigger finding, and it is not a settings finding:** the drafting failure mode at every temperature is not invention from nothing, it is **joining** — building one sentence out of two facts, where every word traces to the record and the sentence is still untrue ("one recruiting *oncology* trial *in Istanbul*", where neither the trial's area nor its location is known). Temperature does not fix this and neither does a prompt rule stated once at the top. What fixed it was changing the shape of the fact sheet: each fact is handed to the model with its own boundary sentence attached ("The company is based in Istanbul. This says nothing about where any trial runs"), plus a one-sentence-one-fact rule. Any future generation node that hands the model several facts about the same entity should assume joining, not fabrication, is the thing to design against.
