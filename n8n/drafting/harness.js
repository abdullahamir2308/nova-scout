// Runs a Workflow 4 Code-node body standalone with a mocked n8n context, so the
// exact JS that ships inside drafting.json is what the tests exercise.
//
// `transform` mirrors the one substitution build_workflow.py makes on the way
// in (the __SIGNATURE__ build constant). Without it the tests would exercise a
// different string than the workflow ships.
//
